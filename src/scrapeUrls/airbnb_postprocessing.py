...
def count_beds(bed_str):
    ...
    normalized_str = (
        bed_str.replace("lits doubles", "lit double")
    ...
    beds = re.findall(
        r"(\d+) (lit double|canapé convertible|lit simple|canapé|lit superposé)",
        normalized_str,
    )
    ...
    return [
        bed_counts["lit double"],
        bed_counts["canapé convertible"],
        bed_counts["lit simple"],
        bed_counts["canapé"],
        bed_counts["lit superposé"],
    ]
...
def process_outputs(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["price"] = data["price"].apply(
        lambda x: extract_number(
            x.replace("€", "").replace("\xa0", "").replace(" ", "")
        )
    )
    data["nb_bedrooms"] = data["nb_bedrooms"].apply(
        lambda x: extract_number(x.split(" ")[0])
    )
    ...
    for feature in equipement_features:
        data[feature] = data["equipements"].apply(
