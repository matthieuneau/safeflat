...
page_nb = 5
url = f"https://www.seloger.com/list.htm?projects=1&types=2%2C1&mandatorycommodities=0&privateseller=1&enterprise=0&qsVersion=1.0&LISTING-LISTpg=1"
driver.get(url)
time.sleep(4)

for i in tqdm(range(1, page_nb + 1), desc="Scraping pages"):
    driver.find_element(
        By.CSS_SELECTOR, "a[data-testid='gsl.uilib.Paging.nextButton']"
    ).click()
    time.sleep(4)
