...
def extract_after_etage(s):
    ...
    if already_formatted_match:
        return (
            s  # The string is already in the desired format, so we return it directly
        )
...
def extract_exposition(text):
    ...
    if match:
        return match.group(
            1
        )  # Group 1 contains the part of the string between "Exposition" and the first comma.
    else:
        return None
...
list_type = [
    ...
    "villa",
    "yacht",
    "villa vacances tout compris",
]


def process_outputs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["balcon"] = df["Extérieur"].apply(
        lambda x: 1 if pd.notnull(x) and "Balcon" in x else 0
    )
    df["terrasse"] = df["Extérieur"].apply(
        lambda x: 1 if pd.notnull(x) and "Terrasse" in x else 0
    )
    df["jardin"] = df["Extérieur"].apply(
        lambda x: 1 if pd.notnull(x) and "Jardin" in x else 0
    )
    df["surface_balcon"] = df["Extérieur"].apply(lambda x: extract_surface(x, "Balcon"))
    df["surface_terrasse"] = df["Extérieur"].apply(
        lambda x: extract_surface(x, "Terrasse")
    )
    ...
    df["meublé"] = df["title"].apply(lambda x: 1 if "meublé" in x else 0)
    df["cave"] = df["Surfaces annexes"].apply(
        lambda x: 1 if pd.notnull(x) and "Cave" in x else 0
    )
    df["parking"] = df["Surfaces annexes"].apply(
        lambda x: 1 if pd.notnull(x) and "Parking" in x else 0
    )
    df["garage"] = df["Surfaces annexes"].apply(
        lambda x: 1 if pd.notnull(x) and "Garage" in x else 0
    )
    df["box"] = df["Surfaces annexes"].apply(
        lambda x: 1 if pd.notnull(x) and "Box" in x else 0
    )
    df["ascenseur"] = df["Services et accessibilité"].apply(check_ascenseur)
    df["ascenseur"] = df["ascenseur"].astype("Int64")
    df["interphone"] = df["Services et accessibilité"].apply(
        lambda x: 1 if pd.notnull(x) and "Interphone" in x else 0
    )
    df["gardien"] = df["Services et accessibilité"].apply(
        lambda x: 1 if pd.notnull(x) and "Gardien" in x else 0
    )
    df["salle de bain (Baignoire)"] = df["Hygiène"].apply(
        lambda x: 1 if pd.notnull(x) and "baignoire" in x else 0
    )
    df["salle d'eau (douche)"] = df["Hygiène"].apply(
        lambda x: 1 if pd.notnull(x) and "douche" in x else 0
    )
    df["surface_salon"] = df["Pièces à vivre"].apply(
        lambda x: extract_surface(x, "Séjour / salon")
    )
    df["surface_salle_a_manger"] = df["Pièces à vivre"].apply(
        lambda x: extract_surface(x, "Salle à manger")
    )
    df["loyer_charges_comprises"] = df["loyer_charges_comprises"].apply(
        lambda x: extract_number(x)
    )
    df["charges_forfaitaires"] = df["charges_forfaitaires"].apply(
        lambda x: extract_number(x)
    )
