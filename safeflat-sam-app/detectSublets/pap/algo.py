...
def filter_and_score(property_infos):
    ...
    for feature, value in property_infos.items():
        if feature in ["nb_rooms", "nb_bedrooms"]:
            data_df = data_df[
                (
                    (max(0, value - 1) <= data_df[feature])
                    & (data_df[feature] <= value + 1)
                )
                | pd.isna(data_df[feature])
            ]  # Only keeps the lines with the corresponding value +-1 or Nan value
        elif feature in ["surface"]:
            if data_df[feature] is not None and value is not None:
                data_df = data_df[
                    (
                        (value * 0.75 <= data_df[feature])
                        & (data_df[feature] <= value * 1.25)
                    )
                ]
...
if __name__ == "__main__":
    ...
    filtered_data.to_csv(
        "C:/Users/hennecol/Documents/safeflat/scraping/pap-oxylab/csv_outputs/output_score.csv"
    )
